  ...
  <div class="experiment-bg">
    ...
    <div class="experiment-create-2col">
      ...
      <div class="exp-card right">
        ...
        <el-form :model="form" label-width="110px" class="form-card" @submit.prevent>
...
const form = ref({
  ...
  videoUrl: '',
  classIds: [] as number[],
  projectType: 'individual', // 默认值为个人项目
...
//   ElMessage.success('实验项目创建成功！（已模拟）')
// }
const submit = async () => {
  try {
    console.log('提交的数据：', form.value);
    ...
    // 带上 Authorization 头，格式是 Bearer + token
    const res = await axios.post(
      '/api/experiment/project/publish',
      form.value,
      {
        headers: {
    ...
    if (res.status === 200) {
      ...
      // 如果是小组实验，调用自动分组接口
      if (form.value.projectType === 'team') {
        const autoGroupRes = await axios.post('/api/projectTeam/auto-group', {
          projectId: res.data.projectId, // ⬅️ 后端记得返回 projectId
          groupSize: 2 
        }, {
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        // 跳转到小组分组展示页面，携带 projectId
        router.push({
          name: 'GroupResult', // ⬅️ 你定义的路由名字
...
.exp-card.right .form-card {
  display: flex;
  flex-direction: column;
  justify-content: center;
  flex: 1 1 0;
}
.exp-card.right .el-form-item:last-child {
  display: flex;
  justify-content: center !important;
  align-items: center;
  width: 100%;
}
.exp-card.right .el-form-item:last-child .el-button {
  margin-left: auto;
  margin-right: auto;
  display: block;
}
